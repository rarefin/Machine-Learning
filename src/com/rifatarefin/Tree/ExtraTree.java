 ...
 */
public class ExtraTree implements TreeInterface {


    private int leafIndex;
    private int seed;
    public ExtraTree(int seed){
        this.seed = seed;
        this.leafIndex = 0;
    }
    ...
    public void createTree(Node root, ArrayList<Double>[] features, ArrayList<Double> labels, ForestSettings forestSettings, int noOfClasses){
        //if(root != null){
          //  root.setEntropy(Stat.getEntropy(labels, noOfClasses));
       // }
        ...
        if(hashSet.size() == 1){
            ...
            HashMap<String, String> bestSplit =  calculateBestSplit(features, labels, forestSettings.getNoOfRandomFeatures(), noOfClasses);
    ...
    private HashMap<String, String> calculateBestSplit(ArrayList<Double>[] features, ArrayList<Double> labels, int noOfRandomFeatures, int noOfClass) {
        ...
        for (int i=0; i<noOfRandomFeatures; i++) {
            ...
            int featureIndex = list.get(index);
            ArrayList<Double> uniqueValues = Stat.getUnique(features[featureIndex]);
            double threshold = uniqueValues.get(random.nextInt(uniqueValues.size()));
            //double threshold = features[featureIndex].get(random.nextInt(features[featureIndex].size()));

            ArrayList<Double>[] childrenOutComes = splitData(features[featureIndex], labels, threshold);
